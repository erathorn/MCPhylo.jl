...
model = Model(
    ...
    (mu, s2) ->
        begin
            sigma = sqrt(s2)
            UnivariateDistribution[NewUnivarDist(mu[i], sigma) for i in 1:length(mu)]
        end,
        false
    ),
    mu = Logical(1,
...
sim = mcmc(model, line, inits, 10000, burnin=250, thin=2, chains=1)
