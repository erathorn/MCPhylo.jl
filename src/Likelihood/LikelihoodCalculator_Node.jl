...
function fels_grad(
    ...
    root_node = tree_postorder[end]
    @inbounds pre_order_partial[:, :, root_node.num] .= pi_
    ptg = Matrix{Float64}(undef, size(U))
    gradi = Array{Float64}(undef, 1, size(data, 2))
    tr = Matrix{Float64}(undef, size(U))
    @inbounds @views for node in reverse(tree_postorder)[2:end]
        ...
        ptg =
            ...
            Uinv
        gradi = mapreduce(*, +, pre_order_partial[:, :, node.num] , (ptg * data[:, :, node.num]), dims = 1, init=0.0)
        ...
        grv[node.num] = mapreduce(/, +, gradi, mapreduce(*, +, pre_order_partial[:, :, node.num], data[:, :, node.num], dims=1, init=0.0), init=0.0)
...
function fels_ll(
    ...
    maxtemp = Array{Float64}(undef, 1, size(data, 2))
    ll::Float64 = 0.0
    @inbounds @views for node in tree_postorder
        if node.nchild > 0
            ...
            if !node.root
                maxtemp = maximum(data[:, :, node.num], dims = 1)
                ll = mapfoldl(log, +, maxtemp, init=ll)
                data[:, :, node.num] ./= maxtemp
                
            else
                
                ll = mapfoldl(log, +, sum(data[:, :, last(tree_postorder).num] .* pi_, dims = 1), init=ll)
            end
        end #if
    end # for
    ll
