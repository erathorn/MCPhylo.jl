...
mt, df = make_tree_with_data("local/development_small.nex"); # load your own nexus file
...
model =  Model(
    ...
    #mtree = Stochastic(Node(), () -> exponentialBL(0.1), true),
...
sim = mcmc(model, my_data, inits, 1000, burnin=50,thin=1, chains=1, trees=true)

# write the output to a path specified as the second argument
to_file(sim, "tneg")

sim1 = read("sim.jls", ModelChains)
