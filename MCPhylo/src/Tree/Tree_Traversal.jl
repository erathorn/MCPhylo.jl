...
"""
    post_order(root::T, traversal::Vector{T})::Vector{T} where T<:Node
...
"""
    post_order(root::T)::Vector{T} where T<:Node
...
"""
    pre_order(root::T, traversal::Vector{T})::Vector{T} where T<:Node
...
"""
    pre_order(root::T)::Vector{T} where T<:Node
...
"""
    level_order(node::T)::Array{T} where T<: Node
...
"""
    level_traverse(node::T, level::Int64, stack::Array{T})::Bool where T <: Node
