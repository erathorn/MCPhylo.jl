...
function Node()::Node
        Node{Float64,Array{Float64,2},Array{Float64,2},Int64}("no_name", ones(3,3), missing,Vector{Node}(undef, 0) ,ones(3),0,true,0.5,"0",1,0.5,nothing,nothing,false)
end

function Node(name::String; data::A)::Node where A<:AbstractArray
        Node{Float64,A,Array{Float64,2},Int64}(name, data ,missing, Vector{Node}(undef, 0), ones(3), 0, true, 0.5, "0", 1, 0.5, nothing, nothing, false)
end
