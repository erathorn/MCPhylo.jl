...
function my_sample!(tree::Array, data_array::Array{Float64,3}, n_leap::Float64, stepsz::Float64, mypi::Number, n_c::Int64, priordist::Distribution)
    delta = 0.01
    rates = ones(n_c)
    surrogate=true
    ...
    probU::Float64 = LogPost(tree, false, delta, rates, n_c, data_array, mypi, priordist)
...
function refraction(tree::Array{Float64,2}, probB::Vector{Float64}, probM::Vector{Float64}, stepsz::Float64, surrogate::Bool,
    n_c::Int64, data_array::Array{Float64,3}, delta::Float64, rates::Vector{Float64}, priordist::Distribution, mypi::Number)
    ...
    while minimum(tmpB)<=0
        ...
        if !(ref_index in leaves)
            # do something
            if surrogate
                U_before_nni::Float64 = LogPost(tree, surrogate, delta, rates, n_c, data_array, mypi, priordist)

                tree_copy = deepcopy(tree)
                tmp_NNI_made = NNI!(tree, ref_index)

                if tmp_NNI_made != 0
                    U_after_NNI::Float64 = LogPost(tree, surrogate, delta, rates, n_c, data_array, mypi, priordist)
...
function LogPost(tree::DenseArray{T}, surrogate::Bool, delta::Float64,
    rates::Vector{Float64}, n_c::Int64, data::Array{Float64}, mypi::Number, priordist::Distribution)::Float64 where {T<:Float64}
    blens = get_branchlength_vector(tree)
    if surrogate
